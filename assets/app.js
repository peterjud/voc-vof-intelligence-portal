...
var firmFilter = "all";
var firmSel = {}; // v2cut -> selected account index (master-detail)
var FORCING={
...
function forcingFn(issue){return FORCING[issue]||"Review this firm's signals and set a next action.";}
...
function renderFirmWatch(){
  ...
  if(!list.length) list=W;
  var sel=firmSel[v2cut]; if(sel==null||sel>=list.length) sel=0;
  var f=list[sel];
  // LEFT — account list (the side drawer)
  var rows=list.map(function(r,i){
    var sent=r.sentiment==="positive"?"green":r.sentiment==="negative"?"red":"neutral";
    return '<button class="acctrow'+(i===sel?" on":"")+'" data-i="'+i+'" type="button">'+
      '<span class="acctdot '+r.signal+'"></span>'+
      '<span class="accttx"><b>'+esc(cleanFirm(r.firm))+'</b><span class="acctsub">'+r.tier+' · '+esc(r.issue)+'</span></span>'+
      '<span class="acctval num">'+(r.dollars?'$'+fmtM(r.dollars):r.deals+'d')+'</span></button>';
  }).join("");
  // RIGHT — briefing for the open account
  var sent=f.sentiment==="positive"?"green":f.sentiment==="negative"?"red":"neutral";
  var top3=(f.top3||[]).map(function(t){return '<span class="sigchip">'+esc(t[0])+' <b>×'+t[1]+'</b></span>';}).join("");
  var brief='<div class="brhead"><div><div class="brfirm">'+esc(cleanFirm(f.firm))+'</div>'+
      '<div class="brtier">'+f.tier+(f.cosell?' · <span class="cosellflag">co-sell</span>':'')+' · Top 100 · '+v2cut+'</div></div>'+
      '<span class="sigbadge '+f.signal+'">'+SIG[f.signal]+'</span></div>'+
    '<div class="brkpis">'+
      '<div class="brk"><span>Calls</span><b class="num">'+f.calls+'</b></div>'+
      '<div class="brk"><span>Lost deals</span><b class="num">'+f.deals+'</b></div>'+
      '<div class="brk"><span>$ at risk</span><b class="num">'+(f.dollars?'$'+fmtM(f.dollars):'—')+'</b></div>'+
      '<div class="brk"><span>Sentiment</span><b class="tag '+sent+'">'+esc(f.sentiment)+'</b></div></div>'+
    '<div class="brblock"><span class="brlab">Top issue &amp; forcing function</span>'+
      '<p class="brissue">'+esc(f.issue)+'</p><p class="brff">'+esc(forcingFn(f.issue))+'</p></div>'+
    (top3?'<div class="brblock"><span class="brlab">Signals in this firm\'s calls</span><div class="chips">'+top3+'</div></div>':'')+
    (f.snippet?'<div class="brblock"><span class="brlab">Call evidence</span><blockquote class="brquote">'+esc(f.snippet)+'</blockquote></div>':'')+
    (f.cosell?'<div class="brwarn">⚠ '+f.cosell_n+' calls flagged co-sell / coaching — the call count is inflated; read the deal count.</div>':'')+
    '<div class="brfoot"><span class="brsrc">Source: Gong closed-lost · '+v2cut+' · directional · signal = churn-driver density</span>'+
      '<button class="brcopy" type="button" data-copy="1">Copy briefing</button></div>';
  var cov=c.deals?Math.round(100*c.dollars_cov/c.deals):0;
  $("#firmWatch").innerHTML=
   '<div class="chart-head"><div><h2 class="h">Accounts — open one to see the briefing</h2><p class="h-sub">'+c.firms+' firms · '+list.length+' shown, worst-first · click an account on the left</p></div>'+
   ...
   '<div class="acctview"><div class="acctlist" role="listbox">'+rows+'</div><div class="acctbrief">'+brief+'</div></div>'+
   '<div class="fnote">$ at risk = Salesforce closed-lost (linked, ~'+cov+'% coverage — a floor). <b>Calls include co-sell/coaching viral calls; deals is the truer count.</b> Diamond firms (T1 with 10+ calls) carry the firm-level read.</div>';
  Array.prototype.slice.call(document.querySelectorAll("#firmWatch [data-ff]")).forEach(function(b){b.addEventListener("click",function(){firmFilter=b.dataset.ff;firmSel[v2cut]=0;renderFirmWatch();});});
  Array.prototype.slice.call(document.querySelectorAll("#firmWatch .acctrow")).forEach(function(b){b.addEventListener("click",function(){firmSel[v2cut]=+b.dataset.i;renderFirmWatch();var bp=$("#firmWatch .acctbrief");if(bp&&window.matchMedia("(max-width:760px)").matches)bp.scrollIntoView({behavior:"smooth",block:"nearest"});});});
  var cp=$("#firmWatch .brcopy"); if(cp){cp.addEventListener("click",function(){
    var txt=cleanFirm(f.firm)+" — "+SIG[f.signal]+" ("+f.tier+", "+v2cut+")\n"+
      f.calls+" calls · "+f.deals+" lost deals · "+(f.dollars?"$"+fmtM(f.dollars)+" at risk":"no linked $")+" · sentiment "+f.sentiment+"\n"+
      "Top issue: "+f.issue+"\nForcing function: "+forcingFn(f.issue)+
      (f.snippet?"\nEvidence: "+f.snippet:"");
    if(navigator.clipboard){navigator.clipboard.writeText(txt).then(function(){cp.textContent="Copied ✓";setTimeout(function(){cp.textContent="Copy briefing";},1500);});}
  });}
}
